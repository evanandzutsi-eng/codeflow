...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!passwordChecks.every(c => c.met)) return setError('Please meet all password requirements.');
      ...
      >
        ...
        <div className={`rounded-2xl p-8 border backdrop-blur-sm ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200 shadow-xl'}`}>
          <div className="text-center mb-8">
            <h1 className={`text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>Create your account</h1>
            <p className={isDark ? 'text-gray-400' : 'text-gray-500'}>Start building smarter with CodeFlow</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className={`absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded ${isDark ? 'text-gray-400 hover:text-white' : 'text-gray-400 hover:text-gray-600'}`}>
              ...
              <div className="mt-3 space-y-1.5">
                {passwordChecks.map((check, i) => (
                  <div key={i} className={`flex items-center space-x-2 text-xs ${check.met ? 'text-green-400' : isDark ? 'text-gray-500' : 'text-gray-400'}`}>
                    <Check className={`w-3.5 h-3.5 ${check.met ? 'opacity-100' : 'opacity-30'}`} />
